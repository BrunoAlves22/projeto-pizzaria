...
describe("SendOrderService", () => {
  ...
  it("deve enviar o pedido e retornar os dados atualizados", async () => {
    const fakeOrder = {
      id: "order-id-1",
      name: "João Silva",
      draft: false,
  ...
  it("deve chamar o prisma atualizando o pedido pelo id informado com draft false", async () => {
    findFirstMock.mockResolvedValue({ id: "order-id-1" } as never);
    updateMock.mockResolvedValue({} as never);

    await service.execute({ name: "João Silva", orderId: "order-id-1" });
  ...
  it("deve lançar AppError 404 se o pedido não existir e não deve atualizar", async () => {
    ...
    await expect(
      service.execute({ name: "João Silva", orderId: "order-inexistente" }),
  ...
  it("deve propagar o erro do prisma se a atualização falhar", async () => {
    ...
    await expect(
      service.execute({ name: "João Silva", orderId: "order-id-1" }),
